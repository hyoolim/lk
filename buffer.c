...
LK_EXT_DEFINIT(lk_buffer_extinittypes) {
    vm->t_buffer = lk_obj_alloc(vm->t_glist);
...
static LK_EXT_DEFCFUNC(at__buf_fi) {
    ...
    RETURN(byte != NULL ? LK_OBJ(lk_fi_new(VM, *byte)) : N);
}
...
LK_EXT_DEFINIT(lk_buffer_extinitfuncs) {
    lk_obj_t *buf = vm->t_buffer, *fi = vm->t_fi;
...
lk_buffer_t *lk_buffer_newfromlist(lk_vm_t *vm, list_t *buf) {
    lk_buffer_t *self = LK_BUFFER(lk_obj_alloc(vm->t_buffer));
