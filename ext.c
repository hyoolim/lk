...
void lk_library_extinit(lk_vm_t *vm) {
    ...
    lk_obj_t *ext = lk_obj_allocWithSize(vm->t_obj, sizeof(lk_library_t));
...
void lk_lib_setObject(lk_obj_t *parent, const char *k, lk_obj_t *v) {
    ...
    lk_str_t *k_kc = lk_str_newFromCString(vm, k);
...
void lk_lib_setCField(lk_obj_t *self, const char *k, lk_obj_t *t,
                   size_t offset) {
