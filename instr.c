...
void lk_instr_libinit(lk_vm_t *vm) {
    ...
    lk_object_set_cfunc_lk(instr, "COLUMN", column, NULL);
    lk_object_set_cfunc_lk(instr, "LINE", line, NULL);
    lk_object_set_cfunc_lk(instr, "MESSAGE", message, NULL);
    lk_object_set_cfunc_lk(instr, "RESOURCE", resource, NULL);
}
