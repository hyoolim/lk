...
static LK_OBJ_DEFALLOCFUNC(alloc__map) {
    qphash_copy(SET(self), SET(parent));
}
...
LK_EXT_DEFINIT(lk_map_extinittypes) {
    ...
    qphash_init(SET(vm->t_map), sizeof(lk_object_t *), lk_object_hashcode, lk_object_keycmp);
...
LK_LIBRARY_DEFINECFUNCTION(at__map_obj) {
    setitem_t *i = qphash_get(SET(self), ARG(0));
    RETURN(i != NULL ? SETITEM_VALUE(lk_object_t *, i) : N);
}
LK_LIBRARY_DEFINECFUNCTION(clearB__map) {
    qphash_clear(SET(self));
    RETURN(self);
}
LK_LIBRARY_DEFINECFUNCTION(size__map) {
    RETURN(lk_fi_new(VM, qphash_size(SET(self))));
}
LK_LIBRARY_DEFINECFUNCTION(keys__map) {
    lk_list_t *keys = lk_list_new(VM);
    SET_EACH(SET(self), i, darray_pushptr(LIST(keys), (void *)i->key));
    RETURN(keys);
}
LK_LIBRARY_DEFINECFUNCTION(setB__map_obj_obj) {
    *(lk_object_t **)qphash_set(SET(self),
...
LK_LIBRARY_DEFINECFUNCTION(values__map) {
    ...
    SET_EACH(SET(self), i,
        darray_pushptr(LIST(values), SETITEM_VALUE(lk_object_t *, i));
    );
    RETURN(values);
}
LK_EXT_DEFINIT(lk_map_extinitfuncs) {
    ...
    lk_library_setCFunction(map, "size", size__map, NULL);
...
lk_object_t *lk_map_get(lk_map_t *self, lk_object_t *k) {
    setitem_t *i = qphash_get(SET(self), k);
