...
static void Dretrieve_obj_str(lk_obj_t *self, lk_scope_t *local) {
    ...
    if (slot != NULL)
        RETURN(lk_obj_getvaluefromslot(self, slot));
    else
        RETURN(NIL);
}
...
static void Dslots_obj(lk_obj_t *self, lk_scope_t *local) {
    ...
    if (self->o.slots != NULL) {
        SET_EACH(self->o.slots, i, darray_ptr_push(DARRAY(slots), (void *)i->key););
    }
    RETURN(slots);
}
...
static void parents_obj(lk_obj_t *self, lk_scope_t *local) {
    if (LK_OBJ_HASPARENTS(self)) {
        ...
        darray_ptr_push(DARRAY(ret), self->o.parent);
        RETURN(ret);
    }
}
static void parent_obj(lk_obj_t *self, lk_scope_t *local) {
    if (LK_OBJ_HASPARENTS(self)) {
        ...
        RETURN(DARRAY_COUNT(pars) > 0 ? darray_ptr_get(pars, -1) : NIL);
    }
...
lk_obj_t *lk_obj_alloc_withsize(lk_obj_t *parent, size_t s) {
    ...
    self->o.tag = tag;
    if (tag->allocfunc != NULL)
        tag->allocfunc(self, parent);
    if (gc != NULL) {
        gc->newvalues++;
...
lk_obj_t *lk_obj_clone(lk_obj_t *self) {
    ...
    if (c->o.tag->allocfunc != NULL)
        c->o.tag->allocfunc(c, self);
    return c;
}
void lk_obj_justfree(lk_obj_t *self) {
    struct lk_tag *tag = self->o.tag;
    if (tag->freefunc != NULL)
        tag->freefunc(self);
    if (LK_OBJ_HASPARENTS(self))
        darray_free(LK_OBJ_PARENTS(self));
    if (self->o.ancestors != NULL)
        darray_free(self->o.ancestors);
    if (self->o.slots != NULL)
...
struct lk_slot *lk_obj_setslot(lk_obj_t *self, lk_obj_t *k, lk_obj_t *check, lk_obj_t *v) {
    ...
    if (slot == NULL) {
        uint32_t first = darray_str_get(DARRAY(k), 0);
        if (self->o.slots == NULL) {
            self->o.slots = qphash_alloc(sizeof(struct lk_slot), lk_obj_hashcode, lk_obj_keycmp);
        }
...
struct lk_slot *lk_obj_setslotbycstr(lk_obj_t *self, const char *k, lk_obj_t *check, lk_obj_t *v) {
    return lk_obj_setslot(self, LK_OBJ(lk_str_new_fromcstr(LK_VM(self), k)), check, v);
}
...
int lk_obj_calcancestors(lk_obj_t *self) {
    lk_obj_t *cand = NULL;
    darray_t *pars;
    darray_t *il, *newancs = darray_ptr_alloc(), *ol = darray_ptr_alloc();
    int candi, j, k, olc, ilc;
    il = darray_ptr_alloc();
    darray_ptr_push(il, self);
    darray_ptr_push(ol, il);
    if (LK_OBJ_HASPARENTS(self)) {
        pars = LK_OBJ_PARENTS(self);
        for (candi = 0; candi < DARRAY_COUNT(pars); candi++) {
            cand = DARRAY_ATPTR(pars, candi);
            il = darray_ptr_alloc();
            if (cand->o.ancestors == NULL)
                lk_obj_calcancestors(cand);
            if (cand->o.ancestors == NULL)
                return 0;
    ...
    if (LK_OBJ_HASPARENTS(self)) {
        ...
        if (self->o.parent != NULL) {
            il = darray_ptr_alloc();
            darray_ptr_push(il, self->o.parent);
            darray_ptr_push(ol, il);
        }
    }
startover:
    for (candi = 0, olc = DARRAY_COUNT(ol); candi < olc; candi++) {
        il = DARRAY_ATPTR(ol, candi);
        if (DARRAY_COUNT(il) < 1)
            continue;
        cand = DARRAY_ATPTR(il, 0);
        for (j = 0; j < olc; j++) {
            il = DARRAY_ATPTR(ol, j);
            for (k = 1, ilc = DARRAY_COUNT(il); k < ilc; k++) {
                if (cand == DARRAY_ATPTR(il, k)) {
                    cand = NULL;
                    goto nextcandidate;
                }
            }
        }
        darray_ptr_push(newancs, cand);
        for (j = 0; j < olc; j++) {
            il = DARRAY_ATPTR(ol, j);
            for (k = 0, ilc = DARRAY_COUNT(il); k < ilc; k++) {
                if (cand == darray_ptr_get(il, k))
                    darray_ptr_remove(il, k--);
            }
        }
        goto startover;
    nextcandidate:;
    }
    for (j = 0; j < DARRAY_COUNT(ol); j++)
        darray_free(DARRAY_ATPTR(ol, j));
    darray_free(ol);
    if (cand == NULL)
        return 0;
    else {
...
static lk_cfunc_t *lk_obj_setcfunc(lk_obj_t *self, const char *name, lk_cfunc_lk_t *cfunc, va_list *args) {
    ...
    for (int i = 0;; i++) {
        ...
        darray_ptr_set(cfuncobj->cf.sigs, i, lk_sig_new(vm, NULL, arg));
    }
    return cfuncobj;
}
...
#define FIND(nil, check) \
    ...
    checkancestors: { \
        ...
        int i, c = DARRAY_COUNT(ancs); \
        for (i = 1; i < c; i++) { \
            self = DARRAY_ATPTR(ancs, i); \
            check \
        } \
        return (nil); \
    } \
...
int lk_obj_keycmp(const void *self, const void *other) {
    if (self == other)
        return 0;
    return !DARRAY_EQ(DARRAY(self), DARRAY(other));
}
