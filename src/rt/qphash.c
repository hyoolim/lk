...
static void qphash_resize(qphash_t *self, int ci) {
    ...
    newcap = newdata->cap;
    SET_EACH(
        self, olditem, delta = 1; newitem = SETITEM_AT(newdata, hashfunc(olditem->key, newcap));
        while (newitem->key != NULL && (newitem->key == SETITEM_SKIPKEY || cmpfunc(newitem->key, olditem->key) != 0)) {
