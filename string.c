...
LK_EXT_DEFINIT(lk_string_extinittypes) {
    ...
    darray_fin(LIST(vm->t_string));
    darray_init(LIST(vm->t_string), sizeof(uint8_t), 16);
}

/* ext map - funcs */
LK_LIBRARY_DEFINECFUNCTION(at__str_fi) {
    RETURN(lk_char_new(VM, darray_getuchar(LIST(self), INT(ARG(0)))));
}
LK_LIBRARY_DEFINECFUNCTION(find__str_ch_fi) {
    int i = darray_findChar(LIST(self), CHAR(ARG(0)), INT(ARG(1)));
    if(i >= 0) RETURN(lk_fi_new(VM, i));
    RETURN(N);
}
LK_LIBRARY_DEFINECFUNCTION(find__str_charset_fi) {
    int i = darray_findCharSet(LIST(self), CHARSET(ARG(0)), INT(ARG(1)));
    if(i >= 0) RETURN(lk_fi_new(VM, i));
    RETURN(N);
}
LK_LIBRARY_DEFINECFUNCTION(find__str_str_fi) {
    int i = darray_findDArray(LIST(self), LIST(ARG(0)), INT(ARG(1)));
    if(i >= 0) RETURN(lk_fi_new(VM, i));
    RETURN(N);
}
LK_LIBRARY_DEFINECFUNCTION(setB__str_fi_ch) {
    darray_setuchar(LIST(self), INT(ARG(0)), CHAR(ARG(1)));
    RETURN(self);
}
LK_LIBRARY_DEFINECFUNCTION(setB__str_fi_fi_str) {
    darray_t *x = LIST(self), *y = LIST(ARG(2));
    darray_resizeitem(x, y);
    darray_setrange(x, INT(ARG(0)), INT(ARG(1)), y);
    RETURN(self);
}
LK_LIBRARY_DEFINECFUNCTION(to_character_qphash__str) {
    lk_charset_t *cs = lk_charset_new(VM);
    charset_addDArray(CHARSET(cs), LIST(self));
    RETURN(cs);
}
...
LK_EXT_DEFINIT(lk_string_extinitfuncs) {
    ...
    lk_library_setCFunction(str, "toCharacterSet", to_character_qphash__str, NULL);
...
lk_string_t *lk_string_newfromlist(lk_vm_t *vm, darray_t *list) {
    lk_string_t *self = LK_STRING(lk_object_alloc(vm->t_string));
    darray_copy(LIST(self), list);
    return self;
}
